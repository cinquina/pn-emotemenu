...
function preview.showEmote(data)
    ...
    if options then
        ...
        if options.Flags then
            movementFlag = options.Flags.Stuck and 50 or options.Flags.Move and 51 or options.Flags.Loop and 1 or
            movementFlag
        ...
        if options.Props then
            for i = 1, #options.Props do
                ...
                local object = CreateObject(previewProp.hash, previewCoords.x, previewCoords.y, previewCoords.z, false,
                    false, false)

                SetEntityCollision(object, false, false)
                AttachEntityToEntity(object, preview.ped, GetPedBoneIndex(preview.ped, previewProp.bone),
                    previewProp.placement[1].x, previewProp.placement[1].y, previewProp.placement[1].z,
                    previewProp.placement[2].x, previewProp.placement[2].y, previewProp.placement[2].z, true, true, false,
                    true, 1, true)
    ...
    TaskPlayAnim(preview.ped, dictionaryName, animationName, 2.0, 2.0, duration or -1, movementFlag, 0, false, false,
        false)
    ...
    if secondaryEmote then
        ...
        TaskPlayAnim(preview.ped, secondaryEmote.Dictionary, secondaryEmote.Animation, 2.0, 2.0,
            secondaryEmote.Duration or -1, 51, 0, false, false, false)
